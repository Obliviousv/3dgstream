...
def evaluate_one_frame_only(dataset, opt, pipe, load_iteration, split_name=None):
    """Evaluate a frame without running optimization."""
    gaussians = GaussianModel(dataset.sh_degree, opt.rotate_sh)
    scene = Scene(dataset, gaussians, load_iteration=load_iteration, shuffle=False)
    ...
    if split_name == "test":
        ...
        cameras = scene.getTestCameras() if dataset.eval else scene.getTrainCameras()
    ...
    with torch.no_grad():
        for viewpoint in cameras:
            ...
            gt_image = torch.clamp(viewpoint.original_image.to("cuda"), 0.0, 1.0)
            l1_total += l1_loss(image, gt_image).mean().double().item()
            psnr_total += psnr(image, gt_image).mean().double().item()

    cam_count = len(cameras)
    return {
        "num_cameras": cam_count,
        "l1": l1_total / cam_count,
        "psnr": psnr_total / cam_count,
    }
...
def train_frames(lp, op, pp, args):
    ...
    frame_items = [
        (item, int(pattern.match(item).group(1)))
        for item in sub_paths
        if pattern.match(item)
    ]
    frame_items.sort(key=lambda x: x[1])
    all_frames = [item for item, _ in frame_items]

    # IMPORTANT: frame_start/frame_end are frame IDs (inclusive), not list indices.
    frames = [
        item
        for item, frame_id in frame_items
        if args.frame_start <= frame_id <= args.frame_end
    ]

    if args.eval_start_frame:
        # IMPORTANT: "start frame" evaluation should target frame000000 (init frame).
        start_frame = "frame000000"
        start_candidates = [
            os.path.join(video_path, start_frame),
            os.path.join(os.path.dirname(model_path), start_frame),
        ]
        start_frame_path = next((p for p in start_candidates if os.path.isdir(p)), None)

        if start_frame_path is None and all_frames:
            # Fallback to previous behavior only when frame000000 is unavailable.
            start_frame = all_frames[0]
            start_frame_path = os.path.join(video_path, start_frame)

        if start_frame_path is None:
            print("[START FRAME] skipped: no available frame directory for evaluation.")
        else:
            print(f"Evaluating start frame {start_frame} before frame-by-frame optimization...")

            prev_source_path = getattr(args, "source_path", "")
            args.source_path = start_frame_path

            start_load_iteration = args.first_load_iteration
            start_eval_test = evaluate_one_frame_only(
                lp.extract(args),
                op.extract(args),
                pp.extract(args),
                start_load_iteration,
                split_name="test",
            )
            start_eval_train = evaluate_one_frame_only(
                lp.extract(args),
                op.extract(args),
                pp.extract(args),
                start_load_iteration,
            ...
            args.source_path = prev_source_path

            if start_eval_test is None and start_eval_train is None:
                print(f"[START FRAME {start_frame}] no cameras available for evaluation.")
            else:
                if start_eval_train is not None:
                    print(
                        f"[START FRAME {start_frame}] train cameras: {start_eval_train['num_cameras']}, "
                        f"L1: {start_eval_train['l1']:.6f}, PSNR: {start_eval_train['psnr']:.6f}"
                    )
                if start_eval_test is not None:
                    print(
                        f"[START FRAME {start_frame}] test cameras: {start_eval_test['num_cameras']}, "
                        f"L1: {start_eval_test['l1']:.6f}, PSNR: {start_eval_test['psnr']:.6f}"
                    )
...
def get_cli_provided_dests(parser: ArgumentParser, argv: List[str]) -> Set[str]:
    option_to_dest = {}
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        action='store_true',
        default=True,
        help="Evaluate frame000001 with the init model before frame-wise training.",
    )
    cli_argv = sys.argv[1:]
    args = parser.parse_args(cli_argv)
    cli_provided_dests = get_cli_provided_dests(parser, cli_argv)
    if args.output_path == "":
    ...
    if args.read_config and args.config_path is not None:
        ...
        for key, value in config.items():
            # IMPORTANT: Keep command-line values authoritative when the same key
            # appears in config, so explicit CLI flags are never overwritten.
            if key in cli_provided_dests:
                continue
